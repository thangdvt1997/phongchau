...
export default function SupportTicketDetailPage({ params }: { params: { id: string } }) {
  ...
  const t = useTranslations('accountSupportDetail');
  ...
  if (!ticket) {
    return <p className="mx-auto max-w-3xl px-6 py-10 text-sm text-gray-500">{t('loading')}</p>;
  }
      ...
      <div className="mt-3 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {ticket.ticketNumber} · {tCategory(ticket.category as any)}
            {ticket.order && ` · ${t('orderPrefix')} ${ticket.order.orderNumber}`}
          </p>
        </div>
        <span className="shrink-0 rounded-full bg-brand-50 px-3 py-1 text-xs font-medium text-brand-700">
          {tStatus(ticket.status as any)}
        </span>
      </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-medium text-gray-800">
                {m.isFromStaff
                  ? `${m.sender?.fullName ?? 'Support'} ${t('supportSuffix')}`
                  : m.sender?.fullName ?? t('you')}
              </span>
