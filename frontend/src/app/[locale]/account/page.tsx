  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10 md:py-16">
      ...
      <section className="mt-10">
        <h2 className="text-lg font-semibold text-gray-900">{t('ordersHeading')}</h2>
        {dataLoading ? (
          <p className="mt-2 text-sm text-gray-500">{t('loadingOrders')}</p>
        ) : orders.length === 0 ? (
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">{t('rfqsHeading')}</h2>
          <Link href="/rfq" className="text-sm font-medium text-brand-700 hover:underline">
            {t('newRfq')}
          </Link>
        </div>
        {dataLoading ? (
          <p className="mt-2 text-sm text-gray-500">{t('loadingRfqs')}</p>
        ) : rfqs.length === 0 ? (
          <p className="mt-2 text-sm text-gray-500">{t('noRfqs')}</p>
        ) : (
          <div className="mt-4 divide-y divide-gray-100 rounded-xl2 border border-gray-100 bg-white shadow-card">
            {rfqs.map((r) => (
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">{t('oemHeading')}</h2>
          <Link href="/oem" className="text-sm font-medium text-brand-700 hover:underline">
            {t('newOemRequest')}
          </Link>
        </div>
        {dataLoading ? (
          <p className="mt-2 text-sm text-gray-500">{t('loadingOem')}</p>
        ) : oemRequests.length === 0 ? (
          <p className="mt-2 text-sm text-gray-500">{t('noOem')}</p>
        ) : (
          <div className="mt-4 divide-y divide-gray-100 rounded-xl2 border border-gray-100 bg-white shadow-card">
            {oemRequests.map((r) => (
              <div key={r.id} className="flex items-center justify-between p-4 text-sm">
