...
tar -xzf /root/phongchau_sync7.tar.gz -C /root/phongchau_new
rsync -a --delete \
  --exclude node_modules --exclude .git --exclude .next --exclude dist \
  --exclude backend/uploads --exclude .env \
  --exclude backend/prisma/seed-images --exclude frontend/public/images \
  /root/phongchau_new/ /root/phongchau/
rm -rf /root/phongchau_new /root/phongchau_sync7.tar.gz
